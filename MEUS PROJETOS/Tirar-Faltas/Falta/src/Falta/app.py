...
class CheckBoxTableApp(toga.App):
    def startup(self):
        ...
        self.table = toga.Table(
            headings=['Item', 'Selecionado'],
            style=Pack(flex=1),
        )

        # Adiciona dados e "checkboxes" (usando botões para marcar/desmarcar)
        for i in range(10):
            # Adiciona uma linha com dados e um botão para selecionar
            item_label = toga.Label(f'Item {i}', style=Pack(flex=1))
            select_button = toga.Button('Selecionar', on_press=self.on_button_press)
            self.table.data.append([item_label, select_button])
    ...
    def on_button_press(self, widget):
        ...
        if widget.label == 'Selecionar':
            widget.label = 'Desmarcar'
        else:
            widget.label = 'Selecionar'
        print(f'Botão pressionado: {widget.label}')

def main():
    return CheckBoxTableApp('Tabela com Botões', 'org.example.checkbox')
...
# """
# Este aplicativo é para tiragem de falta no exército
# """
...
# class TirarFaltas(toga.App):
#     def startup(self):
#         """Constrói e exibe o aplicativo Toga."""

#         # Caixa principal do aplicativo
...
#         # Adiciona o botão à caixa principal
#         main_box.add(button)
...
#         self.main_window.info_dialog('Olá', 'Olá, Mundo!')

# def main():
#     return TirarFaltas('Tirar-Faltas', 'com.tirar.faltas')
